...
class ZiWeiEngine:
    VERSION_ID = "v6.39-PRODUCTION-STABILIZED"
    ...
    INNER_PALACES = {"命宮", "財帛宮", "疾厄宮", "官祿宮", "田宅宮", "福德宮"}
    ...
    def get_innate_audit(self):
        ...
        for t, d in dist.items():
            palace_name = d['palace']
            star_name = d['star']
            
            # Scenario Lookup
            scenario = self.STRATEGIC_SCENARIOS.get(palace_name, {}).get(t, {})
            is_inner = palace_name in self.INNER_PALACES
            
            meaning = scenario.get("sop", "待定策略位")
            impact = scenario.get("inner" if is_inner else "outer", "待定影響位")
            
            res.append({
                "header": f"【生年化{t} ➔ {star_name} ({palace_name})】",
